---
description: Deep Wiki Ingest — แปลง conversation → wiki pages ครบชุด (full pages)
---
...
3. `raw/index.md` — ingest registry ของ raw files
4. State file ที่เกี่ยวข้องกับ session domain ถ้ามี (ดูจาก `.claude/agents/secretary.md` domain routing)
...
ก่อนสร้าง inventory หรือเขียน wiki page ใดๆ:

1. **เขียน raw file** — `raw/YYYY-MM-DD-slug.md` (content immutable):
   ```markdown
   ---
   title: "[Title]"
   type: raw
...
- สำคัญพอที่จะมี page เป็นของตัวเองไหม? (เกณฑ์: ถ้าใน session อื่นในอนาคต user น่าจะอยากอ้างอิงถึงมัน → ใช่)
...
- synthesis ที่ user น่าจะ reference กลับมาในอนาคต
...
- ไม่ต้องรอ confirm จาก user — รายงาน inventory แล้วเริ่มทำเลย